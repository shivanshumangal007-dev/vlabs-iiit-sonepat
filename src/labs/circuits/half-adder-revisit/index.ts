...
// ── Half Adder (Revisit) ──────────────────────────────────────────────────
// Same circuit as the original half-adder but with a distinct id for the
// revisit lab. XOR at col 7, AND at col 16. Two LEDs: Sum (green), Carry (yellow).
//
// A + B → Sum (A XOR B) and Carry (A AND B)
//
// Sum path  (top bank, row c):
//   XOR.Y → r_sum (col 22) → led_sum (col 27) → GND
// Carry path (bottom bank, row h):
//   AND.Y → r_carry (col 22) → led_carry (col 27) → GND

export const HalfAdderRevisitCircuit: Circuit = {
