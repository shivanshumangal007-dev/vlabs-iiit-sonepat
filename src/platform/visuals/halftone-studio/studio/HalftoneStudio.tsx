...
import { HALFTONE_EXPORT_NAMES } from '../export/export-names';
import { HALFTONE_EXPORT_PARSING } from '../export/exporter-parsing';
import { HALFTONE_EXPORTERS } from '../export/exporters';
import {
...
function getAssetFilenameFromUrl(assetUrl: string, fallbackFilename: string) {
  ...
  const assetFilename = sanitizedAssetUrl.split('/').findLast(Boolean);
...
export function HalftoneStudio() {
  ...
  const halftoneBySourceModeReference = useRef({
    shape: { ...HALFTONE_STUDIO_DEFAULTS.shapeHalftone },
    image: { ...HALFTONE_STUDIO_DEFAULTS.imageHalftone },
  });
  ...
  const selectedImportedFile = useMemo(
    () =>
      HALFTONE_EXPORTERS.getModelFile(
        selectedShape,
  ...
  const defaultExportName = useMemo(() => {
    ...
    return HALFTONE_EXPORT_PARSING.deriveComponentName(
      selectedShape,
  ...
  const exportArtifactNames = useMemo(
    () =>
      HALFTONE_EXPORT_NAMES.resolveArtifactNames(exportName, defaultExportName),
  ...
  const handleImportPreset = useCallback(async () => {
    ...
    try {
      const preset = HALFTONE_EXPORT_PARSING.parsePreset(
        await presetFile.text(),
      );
      ...
      const nextPreviewDistance =
        preset.previewDistance ?? HALFTONE_FOOTPRINT.referencePreviewDistance;
  ...
  const handleExportReact = useCallback(() => {
    ...
    downloadText(
      `${componentName}.tsx`,
      HALFTONE_EXPORTERS.generateReactComponent(
        state.settings,
        selectedShape,
        componentName,
        {
  ...
  const handleExportHtml = useCallback(async () => {
    ...
    downloadText(
      `${kebabName}.html`,
      await HALFTONE_EXPORTERS.generateStandaloneHtml(
        state.settings,
        selectedShape,
        componentName,
        {
  ...
  return (
    <STUDIO_STYLES.Shell $background={background}>
      ...
      <STUDIO_STYLES.ContentRegion $panelOpen={controlsVisible}>
        <STUDIO_STYLES.Hint $tone={backgroundTone} $visible={state.showHint}>
          {state.settings.sourceMode === 'image'
        ...
        <STUDIO_STYLES.CanvasLayer ref={canvasLayerReference}>
          <HalftoneCanvas
          ...
          />
        </STUDIO_STYLES.CanvasLayer>
      </STUDIO_STYLES.ContentRegion>

      <STUDIO_STYLES.ControlsPositioner>
        <STUDIO_STYLES.ControlsPanelFrame $visible={controlsVisible}>
          <ControlsPanel
          ...
          />
        </STUDIO_STYLES.ControlsPanelFrame>
      </STUDIO_STYLES.ControlsPositioner>

      <STUDIO_STYLES.HiddenFileInput
