...
const VIRTUAL_RENDER_HEIGHT = ${HALFTONE_FOOTPRINT.virtualRenderHeight};
...
${HALFTONE_INTERACTION_RUNTIME_SOURCE.springStep}

${HALFTONE_INTERACTION_RUNTIME_SOURCE.reset}
...
${HALFTONE_MOUNT_FRAGMENTS.sizingHelpers()}
...
${HALFTONE_MOUNT_FRAGMENTS.rendererSetup("settings.animation.followDragEnabled ? 'grab' : 'default'")}
...
${HALFTONE_MOUNT_FRAGMENTS.postScenes()}
  ...
  const renderFrame = (timestamp) => {
    ...
    renderer.render(scene3d, camera);
${HALFTONE_MOUNT_FRAGMENTS.blurPasses()}
  };
...
${HALFTONE_MOUNT_FRAGMENTS.postScenes()}
  ...
  const interaction = createInteractionState();
  const imagePointerFollow = ${HALFTONE_IMAGE_CONSTANTS.pointerFollow};
  const imagePointerVelocityDamping = ${HALFTONE_IMAGE_CONSTANTS.pointerVelocityDamping};
  const imageHoverFadeIn = ${HALFTONE_IMAGE_CONSTANTS.hoverFadeIn};
  const imageHoverFadeOut = ${HALFTONE_IMAGE_CONSTANTS.hoverFadeOut};
